...
@property(nonatomic, assign)
    BOOL sipInitialized; // 当前电话是否已经注册  - 默认NO
...
- (int)answerCall {
  if (_activeSessionId != INVALID_SESSION_ID) {
    ...
    [[QuanYuSocket shared]
        saveLog:@"answerCall"
        message:[NSString stringWithFormat:@"activeSessionId=%ld, ret=%d",
                                           _activeSessionId, i]];
    return i == 0 ? 0 : 1;
  } else {
    [[QuanYuSocket shared] saveLog:@"answerCall"
                           message:@"activeSessionId INVALID, cannot answer"];
    return 1;
  }
}
...
- (void)onLine {
  ...
  if ([userName length] < 1) {
    if ([self.delegate respondsToSelector:@selector
                       (registerSoftPhoneCallback:errorMsg:)]) {
  ...
  if ([SIPServer length] < 1) {
    if ([self.delegate respondsToSelector:@selector
                       (registerSoftPhoneCallback:errorMsg:)]) {
      [self.delegate registerSoftPhoneCallback:1
                                      errorMsg:@"Please enter SIP Server!"];
    }
    return;
  }
  ...
  [settings setObject:[NSString stringWithFormat:@"%d", mSIPServerPort]
               forKey:@"kSIPServerPort"];
  ...
  int ret = [_portSIPSDK initialize:transport
    ...
    if ([self.delegate respondsToSelector:@selector
      ...
      [self.delegate
          registerSoftPhoneCallback:1
                           errorMsg:[NSString
                                        stringWithFormat:@"initialize failure "
                                                         @"ErrorCode = %d",
                                                         ret]];
    }
  ...
  [[QuanYuSocket shared]
      saveLog:@"PortSipSdk-result"
      message:[NSString stringWithFormat:@"软电话返回：%d", ret]];
  ...
  int rt = [_portSIPSDK
      setLicenseKey:@"1iOS3h05MjdEOUU4NkRBMTdCOTk5QTE1QTg0NzkwRUNGNDJCREA0NzdCM"
  ...
  if (rt != 0) {
    if ([self.delegate respondsToSelector:@selector(pushAppLogToWeb:info:)]) {
      [self.delegate
          pushAppLogToWeb:@"错误"
                     info:[NSString
                              stringWithFormat:@"本机授权失败 ErrorCode = %d",
                                               rt]];
    }
  ...
  [_portSIPSDK setVideoBitrate:-1
                   bitrateKbps:500]; // Default video send bitrate,500kbps
  [_portSIPSDK setVideoFrameRate:-1
                       frameRate:10]; // Default video frame rate,10
  [_portSIPSDK setVideoResolution:352 height:288];
  [_portSIPSDK setAudioSamples:20 maxPtime:60]; // ptime 20

  [_portSIPSDK setInstanceId:[[[UIDevice currentDevice] identifierForVendor]
                                 UUIDString]];
      ...
      [self.delegate
          registerSoftPhoneCallback:1
                           errorMsg:
                               [NSString
                                   stringWithFormat:
                                       @"registerServer failure ErrorCode = %d",
                                       ret]];
    }
  ...
  if (mSIPServerPort == 5060) {
    _sipURL =
        [[NSString alloc] initWithFormat:@"sip:%@:%@", userName, userDomain];
  } else {
    _sipURL = [[NSString alloc]
        initWithFormat:@"sip:%@:%@:%d", userName, userDomain, mSIPServerPort];
  }
...
- (void)offLine {
  ...
  [[QuanYuSocket shared]
      saveLog:@"reset_sessions"
      message:@"offLine reset activeSessionId and lineSessions"];
}
...
- (void)refreshRegister {
  ...
  if (_sipRegistrationStatus == 0) { // 0 - 未注册
    ...
    if ([self.delegate respondsToSelector:@selector(pushAppLogToWeb:info:)]) {
      [self.delegate pushAppLogToWeb:@"Refresh"
...
- (void)unRegister {
  if (_sipRegistrationStatus == 1 || _sipRegistrationStatus == 2) {
    ...
    if ([self.delegate respondsToSelector:@selector(pushAppLogToWeb:info:)]) {
      [self.delegate pushAppLogToWeb:@"unRegister"
                                info:@"unRegister when background"];
    }
...
- (BOOL)setSpeakerEnabled:(BOOL)enabled {
  ...
  [[QuanYuSocket shared] saveLog:@"setAudioDevice"
...
- (void)onRegisterSuccess:(char *)statusText
  ...
  if ([self.delegate
          respondsToSelector:@selector(registerSoftPhoneCallback:errorMsg:)]) {
...
- (void)onInviteClosed:(long)sessionId {
  ...
  [[QuanYuSocket shared]
      saveLog:@"FreeLine"
      message:[NSString stringWithFormat:@"onInviteClosed free sessionId=%ld",
                                         sessionId]];

  NSString *jsString = [NSString
      stringWithFormat:@"onInviteClosed('{\"sessionId\":\"%ld\"}')", sessionId];
...
- (void)onInviteIncoming:(long)sessionId
  ...
  if (index < 0) {
    [[QuanYuSocket shared]
        ...
        message:[NSString stringWithFormat:
                              @"No idle line, reject incoming. sessionId=%ld",
                              sessionId]];
  ...
  [dic setObject:[NSString stringWithFormat:@"%s", callerDisplayName]
          forKey:@"callerDisplayName"];
  [dic setObject:[NSString stringWithFormat:@"%s", caller] forKey:@"caller"];
  [dic setObject:[NSString stringWithFormat:@"%s", calleeDisplayName]
          forKey:@"calleeDisplayName"];
  [dic setObject:[NSString stringWithFormat:@"%s", callee] forKey:@"callee"];
  [dic setObject:[NSString stringWithFormat:@"%s", audioCodecs]
          forKey:@"audioCodecs"];
  [dic setObject:[NSString stringWithFormat:@"%s", videoCodecs]
          forKey:@"videoCodecs"];
  [dic setObject:[NSString stringWithFormat:@"%s", sipMessage]
          forKey:@"sipMessage"];
  ...
  NSString *jsString =
      [NSString stringWithFormat:@"onInviteIncoming('%@')", jsonStr];
  ...
  // 判断自动接听
  BOOL autoAnswer = [[AccountManager sharedAccountManager] AutoAnswerCall];
  int regStatus = _sipRegistrationStatus;
  [[QuanYuSocket shared]
      saveLog:@"AutoAnswer"
      message:[NSString
                  stringWithFormat:
                      @"[Decision]incoming sessionId=%ld, existsAudio=%d, "
                      @"existsVideo=%d, idleLineIndex=%d, autoAnswer=%@",
  ...
  if (autoAnswer && regStatus == 2) {
    int ret = [_portSIPSDK answerCall:sessionId videoCall:existsVideo];
    [[QuanYuSocket shared]
        saveLog:@"AutoAnswer"
        message:[NSString stringWithFormat:@"[Result]sessionId=%ld, ret=%d",
                                           sessionId, ret]];
    if (ret != 0) {
      // 自动接听失败，保留线路供人工接听
      [[QuanYuSocket shared]
          saveLog:@"AutoAnswer"
          message:[NSString
                      stringWithFormat:@"[Decision]auto answer failed, keep "
                                       @"line for manual. sessionId=%ld",
                                       sessionId]];
    }
    return;
  } else if (autoAnswer && regStatus != 2) {
    [[QuanYuSocket shared]
        saveLog:@"AutoAnswer"
        message:[NSString
                    stringWithFormat:@"[Decision]skip auto answer due to "
...
- (void)onACTVTransferFailure:(long)sessionId
                       reason:(char *)reason
                         code:(int)code {
  NSLog(@"onACTVTransferFailure: sessionId=%ld, reason=%s, code=%d", sessionId,
        reason ? reason : "null", code);
  [[QuanYuSocket shared]
      saveLog:@"onACTVTransferFailure"
      message:[NSString stringWithFormat:@"sessionId=%ld, reason=%s, code=%d",
                                         sessionId, reason ? reason : "null",
                                         code]];
}
...
- (void)onAudioRawCallback:(long)sessionId
  ...
  [[QuanYuSocket shared]
      ...
      message:[NSString
                  stringWithFormat:
                      @"sessionId=%ld, mode=%d, dataLength=%d, samplingFreq=%d",
                      sessionId, audioCallbackMode, dataLength,
                      samplingFreqHz]];
}
...
- (void)onDialogStateUpdated:(char *)BLFMonitoredUri
  ...
  [[QuanYuSocket shared]
      ...
      message:[NSString
                  stringWithFormat:@"Uri=%s, State=%s, Id=%s, Direction=%s",
                                   BLFMonitoredUri ? BLFMonitoredUri : "null",
                                   BLFDialogState ? BLFDialogState : "null",
                                   BLFDialogId ? BLFDialogId : "null",
                                   BLFDialogDirection ? BLFDialogDirection
                                                      : "null"]];
}

- (void)onInviteBeginingForward:(char *)forwardTo {
  NSLog(@"onInviteBeginingForward: forwardTo=%s",
        forwardTo ? forwardTo : "null");
  [[QuanYuSocket shared]
      saveLog:@"onInviteBeginingForward"
      message:[NSString stringWithFormat:@"forwardTo=%s",
                                         forwardTo ? forwardTo : "null"]];
}
...
- (void)onInviteFailure:(long)sessionId
                 reason:(char *)reason
                   code:(int)code
             sipMessage:(char *)sipMessage {
  NSLog(@"onInviteFailure: sessionId=%ld, reason=%s, code=%d", sessionId,
        reason ? reason : "null", code);
  [[QuanYuSocket shared]
      saveLog:@"onInviteFailure"
      message:[NSString stringWithFormat:@"sessionId=%ld, reason=%s, code=%d",
                                         sessionId, reason ? reason : "null",
                                         code]];
  ...
  [[QuanYuSocket shared]
      saveLog:@"FreeLine"
      message:[NSString stringWithFormat:@"onInviteFailure free sessionId=%ld",
                                         sessionId]];
}
...
- (void)onInviteRinging:(long)sessionId
             ...
             statusCode:(int)statusCode
             sipMessage:(char *)sipMessage {
  NSLog(@"onInviteRinging: sessionId=%ld, statusText=%s, statusCode=%d",
        sessionId, statusText ? statusText : "null", statusCode);
  [[QuanYuSocket shared]
      saveLog:@"onInviteRinging"
      message:[NSString stringWithFormat:
                            @"sessionId=%ld, statusText=%s, statusCode=%d",
                            sessionId, statusText ? statusText : "null",
                            statusCode]];
}
...
- (void)onInviteSessionProgress:(long)sessionId
  ...
  NSLog(@"onInviteSessionProgress: sessionId=%ld, audioCodecs=%s, "
        ...
        videoCodecs ? videoCodecs : "null", existsEarlyMedia, existsAudio,
        existsVideo);
  [[QuanYuSocket shared]
      saveLog:@"onInviteSessionProgress"
      message:[NSString stringWithFormat:
                            @"sessionId=%ld, audioCodecs=%s, videoCodecs=%s, "
...
- (void)onPlayAudioFileFinished:(long)sessionId fileName:(char *)fileName {
  NSLog(@"onPlayAudioFileFinished: sessionId=%ld, fileName=%s", sessionId,
        fileName ? fileName : "null");
  [[QuanYuSocket shared]
      saveLog:@"onPlayAudioFileFinished"
      message:[NSString stringWithFormat:@"sessionId=%ld, fileName=%s",
                                         sessionId,
                                         fileName ? fileName : "null"]];
}
...
- (void)onPlayVideoFileFinished:(long)sessionId {
  ...
  [[QuanYuSocket shared]
      saveLog:@"onPlayVideoFileFinished"
      message:[NSString stringWithFormat:@"sessionId=%ld", sessionId]];
}

- (void)onPresenceOffline:(char *)fromDisplayName from:(char *)from {
  NSLog(@"onPresenceOffline: fromDisplayName=%s, from=%s",
        fromDisplayName ? fromDisplayName : "null", from ? from : "null");
  [[QuanYuSocket shared]
      saveLog:@"onPresenceOffline"
      message:[NSString
                  stringWithFormat:@"fromDisplayName=%s, from=%s",
...
- (void)onPresenceOnline:(char *)fromDisplayName
  ...
  [[QuanYuSocket shared]
      saveLog:@"onPresenceOnline"
      message:[NSString
                  stringWithFormat:@"fromDisplayName=%s, from=%s, stateText=%s",
                                   fromDisplayName ? fromDisplayName : "null",
                                   from ? from : "null",
                                   stateText ? stateText : "null"]];
}
...
- (void)onPresenceRecvSubscribe:(long)subscribeId
  ...
  [[QuanYuSocket shared]
      saveLog:@"onPresenceRecvSubscribe"
      message:[NSString
                  stringWithFormat:@"subscribeId=%ld, fromDisplayName=%s, "
                                   @"from=%s, subject=%s",
                                   subscribeId,
                                   fromDisplayName ? fromDisplayName : "null",
                                   from ? from : "null",
...
- (void)onReceivedRTPPacket:(long)sessionId
  ...
  NSLog(@"onReceivedRTPPacket: sessionId=%ld, isAudio=%d, packetSize=%d",
        sessionId, isAudio, packetSize);
  [[QuanYuSocket shared]
      saveLog:@"onReceivedRTPPacket"
      message:[NSString
                  stringWithFormat:@"sessionId=%ld, isAudio=%d, packetSize=%d",
                                   sessionId, isAudio, packetSize]];
}
...
- (void)onReceivedRefer:(long)sessionId
  ...
  NSLog(@"onReceivedRefer: sessionId=%ld, referId=%ld, to=%s, from=%s",
        sessionId, referId, to ? to : "null", from ? from : "null");
  [[QuanYuSocket shared]
      saveLog:@"onReceivedRefer"
      message:[NSString stringWithFormat:
                            @"sessionId=%ld, referId=%ld, to=%s, from=%s",
                            sessionId, referId, to ? to : "null",
                            from ? from : "null"]];
}

- (void)onReceivedSignaling:(long)sessionId message:(char *)message {
  NSLog(@"onReceivedSignaling: sessionId=%ld, message=%s", sessionId,
        message ? message : "null");
  [[QuanYuSocket shared]
      saveLog:@"onReceivedSignaling"
      message:[NSString stringWithFormat:@"sessionId=%ld, message=%s",
                                         sessionId,
                                         message ? message : "null"]];
}
...
- (void)onRecvDtmfTone:(long)sessionId tone:(int)tone {
  ...
  [[QuanYuSocket shared]
      saveLog:@"onRecvDtmfTone"
      message:[NSString
                  stringWithFormat:@"sessionId=%ld, tone=%d", sessionId, tone]];
}
...
- (void)onRecvInfo:(char *)infoMessage {
  ...
  [[QuanYuSocket shared]
      saveLog:@"onRecvInfo"
      message:[NSString stringWithFormat:@"infoMessage=%s",
...
- (void)onRecvNotifyOfSubscription:(long)subscribeId
  ...
  [[QuanYuSocket shared]
      ...
      message:[NSString stringWithFormat:
                            @"subscribeId=%ld, notifyMessage=%s, dataLength=%d",
                            subscribeId, notifyMessage ? notifyMessage : "null",
                            messageDataLength]];
}

- (void)onRecvOptions:(char *)optionsMessage {
  NSLog(@"onRecvOptions: optionsMessage=%s",
        optionsMessage ? optionsMessage : "null");
  [[QuanYuSocket shared]
      saveLog:@"onRecvOptions"
      message:[NSString
                  stringWithFormat:@"optionsMessage=%s",
                                   optionsMessage ? optionsMessage : "null"]];
}
...
- (void)onReferAccepted:(long)sessionId {
  ...
  [[QuanYuSocket shared]
      saveLog:@"onReferAccepted"
      message:[NSString stringWithFormat:@"sessionId=%ld", sessionId]];
}

- (void)onReferRejected:(long)sessionId reason:(char *)reason code:(int)code {
  NSLog(@"onReferRejected: sessionId=%ld, reason=%s, code=%d", sessionId,
        reason ? reason : "null", code);
  [[QuanYuSocket shared]
      saveLog:@"onReferRejected"
      message:[NSString stringWithFormat:@"sessionId=%ld, reason=%s, code=%d",
                                         sessionId, reason ? reason : "null",
                                         code]];
}
...
- (void)onSendMessageFailure:(long)sessionId
                   messageId:(long)messageId
                      reason:(char *)reason
                        code:(int)code {
  NSLog(
      @"onSendMessageFailure: sessionId=%ld, messageId=%ld, reason=%s, code=%d",
      sessionId, messageId, reason ? reason : "null", code);
  [[QuanYuSocket shared]
      saveLog:@"onSendMessageFailure"
      message:[NSString stringWithFormat:
                            @"sessionId=%ld, messageId=%ld, reason=%s, code=%d",
                            sessionId, messageId, reason ? reason : "null",
                            code]];
}

- (void)onSendMessageSuccess:(long)sessionId messageId:(long)messageId {
  NSLog(@"onSendMessageSuccess: sessionId=%ld, messageId=%ld", sessionId,
        messageId);
  [[QuanYuSocket shared]
      saveLog:@"onSendMessageSuccess"
      message:[NSString stringWithFormat:@"sessionId=%ld, messageId=%ld",
                                         sessionId, messageId]];
}
...
- (void)onSendOutOfDialogMessageFailure:(long)messageId
  ...
  NSLog(@"onSendOutOfDialogMessageFailure: messageId=%ld, fromDisplayName=%s, "
        ...
        messageId, fromDisplayName ? fromDisplayName : "null",
        from ? from : "null", toDisplayName ? toDisplayName : "null",
        to ? to : "null", reason ? reason : "null", code);
  [[QuanYuSocket shared]
      saveLog:@"onSendOutOfDialogMessageFailure"
      message:[NSString stringWithFormat:
                            @"messageId=%ld, fromDisplayName=%s, from=%s, "
...
- (void)onSendOutOfDialogMessageSuccess:(long)messageId
  ...
  [[QuanYuSocket shared]
      ...
      message:[NSString
                  stringWithFormat:@"messageId=%ld, fromDisplayName=%s, "
                                   @"from=%s, toDisplayName=%s, to=%s",
                                   messageId,
                                   fromDisplayName ? fromDisplayName : "null",
                                   from ? from : "null",
...
- (void)onSendingRTPPacket:(long)sessionId
                 ...
                 RTPPacket:(unsigned char *)RTPPacket
                packetSize:(int)packetSize {
  NSLog(@"onSendingRTPPacket: sessionId=%ld, isAudio=%d, packetSize=%d",
        sessionId, isAudio, packetSize);
  [[QuanYuSocket shared]
      saveLog:@"onSendingRTPPacket"
      message:[NSString
                  stringWithFormat:@"sessionId=%ld, isAudio=%d, packetSize=%d",
                                   sessionId, isAudio, packetSize]];
}
...
- (void)onSubscriptionFailure:(long)subscribeId statusCode:(int)statusCode {
  ...
  [[QuanYuSocket shared]
      ...
      message:[NSString
                  stringWithFormat:@"onSubscriptionFailure:%ld statusCode:%d",
                                   subscribeId, statusCode]];
}

- (void)onSubscriptionTerminated:(long)subscribeId {
  NSLog(@"onSubscriptionTerminated:%ld", subscribeId);
  [[QuanYuSocket shared]
      saveLog:@"onSubscriptionTerminated"
      message:[NSString stringWithFormat:@"onSubscriptionTerminated:%ld",
                                         subscribeId]];
}
